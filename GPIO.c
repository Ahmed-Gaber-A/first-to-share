...
#define GPIO_INT_SENCE_MASK (1 << GPIO_INT_SENCE_BIT_NUM)
...
GPIO_CheckType GPIO_SetInterruptEvent(uint8_t GroupId,GPIO_IntEventType IntEvent,GPIO_IntMaskStatus IntMaskStatus)
{
	...
	if(GroupId < GPIO_GROUPS_NUMBER)
	{
		if(GPIO_GroupState[GroupId] == 0x01)
		{
			...
			{
				PinNumber = GetPinNumber(CfgPtr->PortMask);
				GPIOIM_REG(CfgPtr->PortId) &= ~(CfgPtr->PortMask);
				GPIOIS_REG(CfgPtr->PortId) &= ~(CfgPtr->PortMask);
				GPIOIS_REG(CfgPtr->PortId) |= ((IntEvent & GPIO_INT_SENCE_MASK) >> GPIO_INT_SENCE_BIT_NUM) << PinNumber;
				GPIOIEV_REG(CfgPtr->PortId) &= ~(CfgPtr->PortMask);
				GPIOIEV_REG(CfgPtr->PortId) |= ((IntEvent & GPIO_INT_EVENT_MASK) >> GPIO_INT_EVENT_BIT_NUM) << PinNumber;
				GPIOIBE_REG(CfgPtr->PortId) &= ~(CfgPtr->PortMask);
				GPIOIBE_REG(CfgPtr->PortId) |= ((IntEvent & GPIO_INT_BR_MASK) >> GPIO_INT_BE_BIT_NUM) << PinNumber;
				GPIO_ClrInterruptFlag(GroupId);
				GPIOIM_REG(CfgPtr->PortId) |= IntMaskStatus & CfgPtr->PortMask;
				RetVar = GPIO_OK;
			}
			else
			{
...
GPIO_CheckType GPIO_ClrInterruptFlag(uint8_t GroupId)
{
	...
	if(GroupId < GPIO_GROUPS_NUMBER)
	{
		if(GPIO_GroupState[GroupId] == 0x01)
		{
			...
			if(CfgPtr->SetPinType  != 0x00)
			{
				GPIOICR_REG(CfgPtr->PortId) |= CfgPtr->PortMask;
				RetVar = GPIO_OK;
			}
			else
			{
...
GPIO_CheckType GPIO_GetInterruptStatus(uint8_t GroupId,GPIO_IntStatus *IntStatusPtr)
{
	...
	if(GroupId < GPIO_GROUPS_NUMBER)
	{
		if(GPIO_GroupState[GroupId] == 0x01)
		{
			...
			if(CfgPtr->SetPinType  != 0x00)
			{
				*IntStatusPtr = (GPIORIS_REG(CfgPtr->PortId) & CfgPtr->PortMask) != 0x00 ? INT_PENDING : INT_INACTIVE;
				RetVar = GPIO_OK;
			}
			else
			{
